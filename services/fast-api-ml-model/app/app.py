...
from fastapi import APIRouter, Depends, Request, status, Header, BackgroundTasks
...
from app.utils import get_username_by_token, check_permission
...
async def train_model(model_id: str,
    ...
    username = get_username_by_token(token=token)
    if not check_permission(username=username, model_id=model_id):
        return JSONResponse(content={"message": "User not allowed to train this model."}, status_code=400)
